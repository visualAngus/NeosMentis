...
document.querySelector('.show_demande').addEventListener('click', async () => {
    ...
    if (document.querySelector('.show_demande').innerHTML == 'Show my collaborators') {
        ...
        document.getElementsByClassName('input_research')[0].setAttribute('placeholder', 'Are you looking for a new collaborator ?');
...
async function get_all_actual_collaborators() {
    try {
        ...
        if (data.success) {
            console.log(data);
            document.getElementsByClassName('input_research')[0].setAttribute('placeholder', 'Are you looking for a collaborator ?');
